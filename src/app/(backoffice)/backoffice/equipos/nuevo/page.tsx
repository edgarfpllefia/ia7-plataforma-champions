...
export default function NuevoEquipoPage() {
  ...
  async function onSubmit(formData: FormData) {
    ...
    if (res.ok) {
      ...
      setError(data.error ?? "Error al crear el equipo. Comprueba que tienes permisos.");
    }
  }
  ...
  async function uploadLogo(file: File) {
    ...
    const res = await fetch(`/api/backoffice/teams/${createdId}/logo`, {
      method: "POST",
      body: form,
    });
  ...
  return (
    <main className="container mx-auto max-w-md px-5 py-10">
      <div className="mb-6">
        <h1 className="font-bebas text-4xl tracking-wider">Nuevo Equipo</h1>
      </div>
      ...
      {!createdId ? (
        <div className="bg-white/[0.03] border border-white/8 rounded-3xl p-7">
          <form action={onSubmit} className="space-y-4">
            <div>
              ...
              <input name="name" placeholder="FC Barcelona" required
            ...
            <div>
              ...
              <input name="season" placeholder="2025/26" defaultValue="2025/26" required
                className="w-full bg-white/5 border border-white/8 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-colors" />
            </div>
            <div>
        ...
        <div className="bg-white/[0.03] border border-white/8 rounded-3xl p-7 space-y-5">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center">
              <span className="text-green-400 text-sm">✓</span>
            </div>
            <p className="font-semibold text-green-400">Equipo creado correctamente</p>
          </div>

          <div className="border-t border-white/5 pt-5">
            <label className="block text-[11px] font-bold tracking-[0.15em] uppercase text-gray-500 mb-3">
              Subir logo a Supabase (opcional)
            </label>
            <input
              type="file"
            ...
            />
            {logoUploading && <p className="text-xs text-blue-400 mt-2">Subiendo logo...</p>}
          </div>

          <button
            onClick={() => { router.push("/backoffice/equipos"); router.refresh(); }}
            className="w-full bg-white/5 hover:bg-white/10 border border-white/10 py-3 rounded-xl font-semibold text-sm transition-colors"
          >
